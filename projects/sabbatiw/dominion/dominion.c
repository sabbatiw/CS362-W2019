...
void play_adventurer(struct gameState *state)
{
    ...
    while (drawntreasure < 2)
    {
        ...
        else
        {
            ...
            // this should remove the top card (the most recently drawn one).
...
void play_council_room(struct gameState *state, int handPos)
{
    ...
    for (i = 0; i <= state->numPlayers; i++) // implented for assignment-2
    {
        if (i != currentPlayer)
        {
            drawCard(i, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    switch( card ) 
    {
        ...
        case council_room:
            play_council_room(state, handPos);
            return 0;
        ...
        case remodel:
           // play_remodel returns 0 or -1
           return play_remodel(state, handPos, choice1, choice2);
